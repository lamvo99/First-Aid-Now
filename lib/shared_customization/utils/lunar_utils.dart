...
import 'package:first_aid/model/lunar_date/lunar_date.dart';
...
class LunarUtils {
  static String getLocalizedCan(int index) {
    final list = i18n.Lunar.Can.split(',');
    return list.isNotEmpty ? list[index % list.length] : '';
  }

  static String getLocalizedChi(int index) {
    final list = i18n.Lunar.Chi.split(',');
    return list.isNotEmpty ? list[index % list.length] : '';
  }

  static String getLocalizedTietKhi(int index) {
    final list = i18n.Lunar.SolarTerms.split(',');
    return list.isNotEmpty ? list[index % list.length] : '';
  }
  ...
  static int getLeapMonth(int lunarYear, double timeZone) {
    final a11 = LunarUtils.getLunarMonth11(lunarYear, timeZone);
    final b11 = LunarUtils.getLunarMonth11(lunarYear + 1, timeZone);
    if (b11 - a11 > 365) {
      // Năm có tháng nhuận
      final leapMonth = LunarUtils.getLeapMonthOffset(a11, timeZone) - 2;
  ...
  static String getCanChiYear(int year) {
    final can = getLocalizedCan(year % 10);
    final chi = getLocalizedChi(year % 12);
    return '$can $chi';
  }

  static String getCanChiMonth(
      int month, int year) {
    final chi = i18n.Lunar.Chi.split(',')[month - 1];
    final can = i18n.Lunar.Can.split(',')[year % 10];
    return '$can $chi';
  }
  ...
  static String getCanHour(int jdn) {
    final can = getLocalizedCan((jdn - 1) * 2 % 10);
    final chi = getLocalizedChi((jdn + 1) % 12);
    return '$can $chi';
  }

  static String getCanDay(int jdn) {
    final can = getLocalizedCan((jdn + 9) % 10);
    final chi = getLocalizedChi((jdn + 1) % 12);
    return '$can $chi';
  }
  ...
  static String getBeginHour(int jdn) {
    final can = getLocalizedCan((jdn - 1) * 2 % 10);
    final chi = getLocalizedChi(0);
    return '$can $chi';
  }
}